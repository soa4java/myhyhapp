...
//	private LinearLayout llAt;
	...
	private TitleLayout getPageTitle() {
		...
		llTitle.setId(1);
		//title的背景
		int resId = getBitmapRes(activity, "index_title_bg");
		if (resId > 0) {
		...
		if (resId > 0) {
			...
			llTitle.getTvTitle().setGravity(Gravity.CENTER_HORIZONTAL|Gravity.CENTER_VERTICAL);
		}
		llTitle.getBtnRight().setVisibility(View.VISIBLE);
		resId = getStringRes(activity, "share");
		if (resId > 0) {
			llTitle.getBtnRight().setText(resId);
			llTitle.getBtnRight().setTextSize(TypedValue.COMPLEX_UNIT_SP, 14);
		}
	...
	private RelativeLayout getThumbView() {
		...
		resId = getBitmapRes(activity, "xpl_del_select");
		if (resId > 0) {
